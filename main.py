...
# Iniciar servicio de reinicio de tareas diarias cuando se inicia la aplicación
try:
    logger.info("Iniciando servicio de reinicio de tareas diarias...")
    from daily_tasks_reset_service import start_daily_tasks_reset_service
    
    # Iniciar el servicio
    success = start_daily_tasks_reset_service()
    if success:
        logger.info("✓ Servicio de reinicio de tareas diarias iniciado correctamente")
    else:
        logger.warning("⚠ No se pudo iniciar el servicio de reinicio de tareas diarias")
except Exception as e:
    logger.error(f"❌ Error al iniciar el servicio de reinicio de tareas diarias: {str(e)}")
